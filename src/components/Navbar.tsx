        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          ...
          {isLoggedIn && user ? (
            <>
              ...
              {/* Profile link — opens /profile/edit. Was previously
                  reachable only by typing the URL; students had no
                  visible affordance for the quick-login password
                  setup card or the LinkedIn-edit form. The student's
                  display name doubles as the link target on md+
                  viewports; on mobile we show a person icon button. */}
              <Link
                href="/profile/edit"
                className="hidden md:flex items-center gap-1 text-[11px] text-zinc-400 hover:text-white transition-colors truncate max-w-[120px] px-1.5 py-1 rounded-lg hover:bg-white/[0.04]"
              ...
              <Link
                ...
                title="Edit profile + set quick-login password"
                className="md:hidden text-[11px] font-medium text-zinc-500 hover:text-white transition-colors p-1.5 rounded-lg hover:bg-white/[0.04] inline-flex items-center"
              >
