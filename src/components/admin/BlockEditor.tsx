...
function EraCardsEditor({
  ...
  function updateCard(i: number, patch: Partial<import("@/types/content").EraCard>) {
    const items = [...block.items];
    items[i] = { ...items[i], ...patch };
    onChange({ ...block, items });
  }
...
// Register era-cards in the insert-block command palette. Adding at
// the bottom rather than alongside "Cards grid" to avoid confusing
// teachers — era-cards is a specialized shape most courses won't need.
BLOCK_MENU.push({
  type: "era-cards",
